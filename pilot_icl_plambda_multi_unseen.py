...
+ Unseen Task Adaptation via Direct Opt(p,lambda) Context

TRAINING:
  Multi-scenario batch-level round-robin training with (p,lambda) beamformer
  structure across 8 channel scenarios (S0-S7). One shared ICL Transformer,
  per-scenario datasets. NO BFEncoder/BFDecoder needed.

UNSEEN TASK ADAPTATION (post-training, no gradient updates):
  For each unseen scenario:
    1. Generate context dataset: channel samples -> MMSE channel estimate ->
       optimize (p*,lambda*) on H_hat -> store (H, p*, lambda*, rate) pairs
    2. For each test query pilot: random context from the dataset, forward
       through frozen model to predict (p_pred, lam_pred), reconstruct W via
       the optimal structure using H_hat. Evaluate on true channel H.
  This is a SINGLE forward pass adaptation — no gradients, no retraining.

ARCHITECTURE:
  PilotEncoder: Y_real -> z in R^{4K}  (CNN + attention pooling)
  ICL Transformer: {z_1, y_1, ..., z_l, y_l, z_q} -> y_hat in R^{4K}
    where y_i = [p_i, lambda_i, 0, 0] is the zero-padded label token
  Output head: y_hat[:K] -> p (sigmoid + L1-norm), y_hat[K:2K] -> lambda (softplus)
...
# Unseen scenarios for post-training adaptation (NOT in training set)
UNSEEN_SCENARIOS = [
    ScenarioDef("Highway",        100, 'sparse', 2, 12, 5.0),
    ScenarioDef("Dense Indoor",   101, 'sparse', 8, 2, 25.0),
    ScenarioDef("Rural LoS",      102, 'sparse', 1, 20, 1.5),
]
...
###############################################################################
# 11. UNSEEN TASK ADAPTATION — Direct Opt(p,lambda) Context
###############################################################################
@torch.no_grad()
def adapt_unseen_direct(model, Phi, cfg, unseen_scenarios=None):
    """
    Unseen Task Adaptation with (p,lambda) Parameterization.
    NO gradient updates. NO parameter modifications. Pure forward passes.

    For each unseen scenario:
      Step 1: Generate context dataset
        - Sample channel realizations from the unseen scenario
        - Compute MMSE channel estimate H_hat from noisy pilots
        - Optimize (p*, lambda*) on H_hat via Adam (robust labels)
        - Store (H, p*, lambda*, rate) as context dataset

      Step 2: Generate held-out test set (pilot signals only at inference)

      Step 3: ICL forward-pass inference
        - For each test query pilot: random context from Step 1 dataset
        - PilotEncoder compresses demo pilots and query pilot to tokens
        - ICL Transformer predicts (p_pred, lam_pred) from context
        - Reconstruct W via optimal structure using query's H_hat
        - Evaluate sum-rate on TRUE channel (for metrics only)
    """
    model.eval()
    if unseen_scenarios is None:
        unseen_scenarios = UNSEEN_SCENARIOS

    print("\n" + "=" * 75)
    print("  UNSEEN TASK ADAPTATION — (p,lambda) Direct Context")
    print("=" * 75)

    results = {}

    for sc in unseen_scenarios:
        print(f"\n--- Unseen Scenario: S{sc.sid} {sc.name} ---")
        tag = f"({sc.n_clusters}cl,{sc.n_rays}r,{sc.spread_deg}°)" if sc.ch_type == 'sparse' else "(iid)"
        print(f"  Channel: {sc.ch_type} {tag}")

        # ---- Step 1: Build context dataset with Opt(p,lambda) labels ----
        print(f"  [Step 1] Generating {cfg.unseen_context_size} context samples...")
        ctx_H = generate_channel_scenario(cfg.unseen_context_size, cfg.K, cfg.N, sc)
        ctx_Y = pilot_observe(ctx_H, Phi, cfg.sigma2)
        ctx_H_hat = mmse_channel_est(ctx_Y, Phi, cfg.sigma2)

        # Optimize (p*, lambda*) on H_hat (robust mode — matches inference reconstruction)
        ctx_p, ctx_lam, _ = generate_optimal_params(
            ctx_H_hat, cfg.P_max, cfg.sigma2, cfg.opt_iters, cfg.opt_lr, cfg.opt_restarts)

        # Evaluate achieved rate on TRUE channel (for quality monitoring)
        ctx_W = reconstruct_precoder(ctx_H_hat, ctx_p, ctx_lam, cfg.sigma2)
        ctx_rate = compute_sum_rate(ctx_H, ctx_W, cfg.sigma2)
        print(f"    Context Opt(p,lam) avg rate: {ctx_rate.mean().item():.3f}")

        # Also compute MMSE-perfect baseline rate on context set for reference
        ctx_mmse_rate = compute_sum_rate(
            ctx_H, mmse_beamformer(ctx_H, cfg.P_max, cfg.sigma2), cfg.sigma2)
        print(f"    Context MMSE-Perfect avg rate: {ctx_mmse_rate.mean().item():.3f}")

        # ---- Step 2: Generate held-out test samples ----
        print(f"  [Step 2] Generating {cfg.unseen_n_test} test samples...")
        test_H = generate_channel_scenario(cfg.unseen_n_test, cfg.K, cfg.N, sc)
        test_Y = pilot_observe(test_H, Phi, cfg.sigma2)
        test_pil = pilot_to_real(test_Y)
        test_H_hat = mmse_channel_est(test_Y, Phi, cfg.sigma2)

        # Baselines on test set
        bl_mmse_perf = compute_sum_rate(
            test_H, mmse_beamformer(test_H, cfg.P_max, cfg.sigma2), cfg.sigma2).mean().item()
        bl_mmse_imp = compute_sum_rate(
            test_H, mmse_beamformer(test_H_hat, cfg.P_max, cfg.sigma2), cfg.sigma2).mean().item()

        # Opt(p,lam) + imperfect CSI baseline
        bl_p, bl_lam, _ = generate_optimal_params(
            test_H_hat, cfg.P_max, cfg.sigma2, cfg.opt_iters, cfg.opt_lr, cfg.opt_restarts)
        bl_W_opt = reconstruct_precoder(test_H_hat, bl_p, bl_lam, cfg.sigma2)
        bl_opt_imp = compute_sum_rate(test_H, bl_W_opt, cfg.sigma2).mean().item()

        print(f"    Baselines: MMSE-P={bl_mmse_perf:.3f}  MMSE-E={bl_mmse_imp:.3f}  "
              f"Opt(p,l)-E={bl_opt_imp:.3f}")

        # ---- Step 3: ICL forward-pass inference ----
        print(f"  [Step 3] ICL forward pass (random context, no gradients)...")
        bs = min(cfg.batch_size, cfg.unseen_n_test)
        all_rates = []

        for s in range(0, cfg.unseen_n_test, bs):
            e = min(s + bs, cfg.unseen_n_test)
            b = e - s

            # Query pilot (no true channel available at inference)
            q_pil = test_pil[s:e]         # (b, 2*N*Lp)
            q_H = test_H[s:e]             # only for evaluation
            q_H_hat = test_H_hat[s:e]     # MMSE estimate for precoder reconstruction

            # Random context from the (p,lambda) dataset
            d_idx = torch.randint(0, cfg.unseen_context_size, (b, cfg.n_demos), device=device)

            # Build demo pilot tokens on-the-fly from stored H
            d_H = ctx_H[d_idx].reshape(b * cfg.n_demos, cfg.K, cfg.N)
            d_Y = pilot_observe(d_H, Phi, cfg.sigma2)
            d_pil = pilot_to_real(d_Y).reshape(b, cfg.n_demos, -1)  # (b, l, 2NLp)
            d_p = ctx_p[d_idx]     # (b, l, K)
            d_lam = ctx_lam[d_idx] # (b, l, K)

            # Forward pass: predict (p, lambda) from context
            p_pred, lam_pred = model(d_pil, d_p, d_lam, q_pil)

            # Reconstruct precoder using MMSE channel estimate (no true H)
            W_hat = reconstruct_precoder(q_H_hat, p_pred, lam_pred, cfg.sigma2)

            # Evaluate sum-rate on TRUE channel
            rate = compute_sum_rate(q_H, W_hat, cfg.sigma2)
            all_rates.append(rate)

        avg_rate = torch.cat(all_rates).mean().item()

        results[sc.sid] = {
            'name': sc.name,
            'icl_rate': avg_rate,
            'mmse_perf': bl_mmse_perf,
            'mmse_imp': bl_mmse_imp,
            'opt_imp': bl_opt_imp,
            'ctx_size': cfg.unseen_context_size,
        }

        print(f"\n  RESULT: ICL rate = {avg_rate:.3f}")
        print(f"    vs MMSE-Perfect      = {bl_mmse_perf:.3f}  ({avg_rate/bl_mmse_perf*100:.1f}%)")
        print(f"    vs MMSE-Imperfect    = {bl_mmse_imp:.3f}  ({avg_rate/bl_mmse_imp*100:.1f}%)")
        print(f"    vs Opt(p,l)-Imperf   = {bl_opt_imp:.3f}  ({avg_rate/bl_opt_imp*100:.1f}%)")

    # Summary table
    print("\n" + "=" * 75)
    print("  UNSEEN TASK ADAPTATION SUMMARY — (p,lambda) Direct Context")
    print("=" * 75)
    print(f"  {'Scenario':<20} {'ICL':>8} {'MMSE-P':>8} {'MMSE-E':>8} {'Opt-E':>8} {'%Opt-E':>8}")
    print("  " + "-" * 60)
    for sid, r in results.items():
        pct = r['icl_rate'] / r['opt_imp'] * 100 if r['opt_imp'] > 0 else 0
        print(f"  {r['name']:<20} {r['icl_rate']:8.3f} {r['mmse_perf']:8.3f} "
              f"{r['mmse_imp']:8.3f} {r['opt_imp']:8.3f} {pct:7.1f}%")
...
if __name__ == "__main__":
    ...
    # Stage 1: Multi-scenario training with (p,lambda) parameterization
    model, mds, bl, Phi = train(cfg)

    # Stage 2: Unseen task adaptation (forward-only, no gradient updates)
    unseen_results = adapt_unseen_direct(model, Phi, cfg)
