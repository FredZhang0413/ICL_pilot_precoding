"""
ABLATION A2c: Hard Switch ICL (No Gradual Curriculum)

PURPOSE: Show that the GRADUAL curriculum transition is important.
This baseline uses the full ICL architecture with Phase 1 MSE warm-start,
but at epoch E1+1 it INSTANTLY switches to 100% unsupervised sum-rate loss
(no gradual r(e) schedule). Self-bootstrapping is active in Phase 2.

EXPECTED RESULT: Training instability at the switch point — the model may
experience catastrophic forgetting of the supervised prior or gradient shock,
leading to worse final performance than the smooth curriculum.
"""
...
# ============================================================================
# SHARED UTILITIES (identical to proposed — compact form)
# ============================================================================
class Config:
    def __init__(self,**kw):
        ...
        self.edn_batch=kw.get('edn_batch',128);self.edn_ds_size=kw.get('edn_ds_size',5000)
        self.n_demos=kw.get('n_demos',5)
        self.d_model=kw.get('d_model',512);self.n_heads=kw.get('n_heads',8)
        self.n_layers=kw.get('n_layers',6);self.d_ff=kw.get('d_ff',1024);self.dropout=kw.get('dropout',0.0)
        ...
        self.initial_ds_size=kw.get('initial_ds_size',1024);self.unsup_scale=kw.get('unsup_scale',0.01)
        self.phase1_epochs=kw.get('phase1_epochs',50);self.phase2_epochs=kw.get('phase2_epochs',500)
        self.total_epochs=self.phase1_epochs+self.phase2_epochs
        self.steps_per_epoch=kw.get('steps_per_epoch',80);self.n_test=kw.get('n_test',200)
        self.max_ds_size=kw.get('max_ds_size',50000)
        self.boot_alpha_start=kw.get('boot_alpha_start',0.60);self.boot_alpha_end=kw.get('boot_alpha_end',0.90)
        self.boot_beta_start=kw.get('boot_beta_start',0.60);self.boot_beta_end=kw.get('boot_beta_end',0.90)
        self.prune_every=kw.get('prune_every',10);self.prune_drop_end=kw.get('prune_drop_end',0.10)
...
class ChannelDecoder(nn.Module):
    def __init__(self,K,N,D_tok,hidden=512):
        super().__init__();self.net=nn.Sequential(nn.Linear(D_tok,hidden),nn.GELU(),nn.Linear(hidden,hidden),nn.GELU(),nn.Linear(hidden,2*K*N))
    def forward(self,z): return self.net(z)
class BFEncoder(nn.Module):
    def __init__(self,N,K,D_tok,hidden=512):
        super().__init__();self.net=nn.Sequential(nn.Linear(2*N*K,hidden),nn.GELU(),nn.Linear(hidden,hidden),nn.GELU())
...
class CausalBlock(nn.Module):
    ...
    def forward(self,x,mask):h=self.ln1(x);x=x+self.attn(h,h,h,attn_mask=mask)[0];return x+self.ff(self.ln2(x))
...
class ICLModelFull(nn.Module):
    ...
    def forward(self,demo_pil,demo_W,query_pil,sigma2=None):
        B,l,_=demo_pil.shape;all_pil=torch.cat([demo_pil.reshape(B*l,-1),query_pil],0)
        with torch.no_grad():
        ...
        out=self.transformer(torch.stack(tokens,dim=1));return self.bf_dec(out[:,-1],normalize=False)
...
# ============================================================================
# A2c TRAINING: HARD SWITCH (MSE → instant 100% sum-rate, no gradual r(e))
# ============================================================================
def train(cfg):
    set_global_seed(cfg.seed)
    print("="*70)
    print("  ABLATION A2c: Hard Switch ICL (No Gradual Curriculum)")
    print("="*70)
    ...
    # Initial labeled dataset (same as proposed)
    print(f"\nGenerating labeled dataset ({cfg.initial_ds_size} samples)...")
    ...
    for epoch in range(cfg.total_epochs):
        ...
        # KEY DIFFERENCE: Phase 1 = pure MSE, Phase 2 = INSTANT 100% unsup
        # No gradual transition — r jumps from 0 to 1.0 at epoch E1+1
        if epoch < cfg.phase1_epochs:
            phase=1; r=0.0
        else:
            phase=2; r=1.0  # <-- HARD SWITCH: 100% unsupervised immediately
        ...
        if sup_pool.numel()==0: sup_pool=torch.arange(ds.size,device=device)

        for step in range(cfg.steps_per_epoch):
            B=cfg.batch_size;l=cfg.n_demos

            if phase==1:
                # Supervised: from labeled dataset
                idx=sup_pool[torch.randint(0,sup_pool.numel(),(B,),device=device)]
                q_H=ds.H[idx]; q_W_gt=ds.W_real[idx]
                q_Y=pilot_observe(q_H,Phi,cfg.sigma2); q_pil=pilot_to_real(q_Y)
            else:
                # 100% unsupervised: fresh channels
                q_H=generate_channel(B,cfg.K,cfg.N,cfg.ch_n_clusters,cfg.ch_n_rays,cfg.ch_spread_deg)
                q_Y=pilot_observe(q_H,Phi,cfg.sigma2); q_pil=pilot_to_real(q_Y)
                q_W_gt=None  # not used
        ...
        if phase==2 and (epoch+1)%cfg.prune_every==0:
            dr=cfg.prune_drop_end*(epoch-cfg.phase1_epochs)/max(1,cfg.phase2_epochs-1)
        ...
        print(f"{epoch+1:3d} ph{phase} r={r:.0%} | mse={ep_mse/max(1,ep_n):.5f} rate={ep_rate/max(1,ep_n):.2f} | "
              f"test={tr:.3f} best={best_test:.3f} | DS={ds.size} +{ep_add} -{nd} ({dt:.1f}s)",flush=True)

    b4=bl['wmmse_imp']
    print("\n"+"="*68+"\n  A2c COMPLETE (Hard Switch)")
    for k,v in [("B1",bl['mmse_perf']),("B2",bl['mmse_imp']),("B3",bl['wmmse_perf']),("B4",b4),("A2c best",best_test)]:
        print(f"  {k:<20} {v:8.3f}  ({100*v/b4:.1f}%)" if b4>0 else f"  {k} {v}")
    return model,bl

if __name__=="__main__":
    cfg=Config(K=32,N=32,L_p=20,SNR_dB=20,D_tok=256,edn_hidden=64,edn_epochs=200,edn_ds_size=5000,
               n_demos=5,d_model=512,n_heads=8,n_layers=6,d_ff=1024,
               initial_ds_size=1024,phase1_epochs=50,phase2_epochs=1000,steps_per_epoch=80,
               batch_size=64,lr=1.5e-4,unsup_scale=0.01,max_ds_size=50000,n_test=200)
    train(cfg)
