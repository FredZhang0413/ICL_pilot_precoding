"""
ABLATION A3b: NO QUALITY THRESHOLD (Accept All Samples)

Full ICL architecture + full curriculum + self-bootstrapping is ACTIVE,
but EVERY unsupervised solution is added to the dataset regardless of quality.
No per-instance threshold, no cross-instance percentile filter.

This proves that quality control is essential: without it, early-training
garbage solutions pollute the context pool, teaching the model bad patterns
and creating a negative feedback loop.

EXPECTED RESULT: Worse than proposed (especially early on), with potential
for divergence or oscillation as low-quality demos corrupt the context.
...
def train(cfg):
    ...
    for epoch in range(cfg.total_epochs):
        ...
        for step in range(cfg.steps_per_epoch):
            ...
            # [A3b] ACCEPT ALL: every unsupervised solution enters the dataset.
            # No per-instance threshold, no cross-instance filter.
            with torch.no_grad():
                if phase == 2:
                    ...
                    if n_cand <= 0:
                        ...
                        # [A3b] NO THRESHOLD — accept ALL candidates unconditionally
                        ds.add(cand_H, cand_pil, bf_to_real(cand_W_hat),
                               cand_rate, mmse_rates=cand_mmse_imp_r, supervised=False)
                        ep_add += n_cand
