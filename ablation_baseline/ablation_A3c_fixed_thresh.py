"""
ABLATION A3c: FIXED THRESHOLD (Non-Adaptive)

Full ICL architecture + full curriculum + self-bootstrapping is ACTIVE,
but the admission threshold is FIXED throughout training (no scheduling).
Uses a single constant: rate > fixed_alpha * MMSE_rate.
No cross-instance percentile filter, no adaptive alpha/beta scheduling.

Two sub-experiments:
  - strict: fixed_alpha = 0.95 (may block almost all samples early on)
  - loose:  fixed_alpha = 0.60 (admits too much garbage early on)
Set via cfg.fixed_alpha (default: 0.95 for strict; change to 0.60 for loose).

EXPECTED RESULT: Strict threshold leads to little/no dataset growth (the model
hasn't learned enough to beat 0.95*MMSE early on). Loose threshold admits
garbage, degrading context quality. The proposed ADAPTIVE dual-threshold
(alpha rising 0.60→0.90, beta rising 0.60→0.90) balances both failure modes.

Based on: Pilot-Based In-Context Learning for MU-MISO Precoding — v5
...
class Config:
    def __init__(self, **kwargs):
        ...
        # [A3c] Fixed threshold: overrides adaptive alpha/beta scheduling
        self.fixed_alpha = kwargs.get('fixed_alpha', 0.95)  # set 0.60 for loose variant
...
def train(cfg):
    ...
    for epoch in range(cfg.total_epochs):
        ...
        for step in range(cfg.steps_per_epoch):
            ...
            # [A3c] FIXED THRESHOLD: rate > fixed_alpha * MMSE_rate (constant, no scheduling)
            # No cross-instance percentile filter. No adaptive alpha/beta.
            with torch.no_grad():
                if phase == 2:
                    ...
                    if n_cand <= 0:
                        ...
                        # [A3c] Single fixed threshold — no scheduling, no cross-instance
                        good = cand_rate > cfg.fixed_alpha * cand_mmse_imp_r
                        if good.any():
            ...
            # phase2 -> average on unsupervised dataset queries.
            rate_mask = q_is_sup_ds if phase == 1 else q_is_unsup_ds
            if rate_mask.any():
