...
class Config:
    def __init__(self, **kwargs):
        ...
        self.new_unsup_rate_ratio = kwargs.get('new_unsup_rate_ratio', 0.5)
        self.loss_mse_weight = kwargs.get('loss_mse_weight', 1.0)
        self.loss_rate_ds_weight = kwargs.get('loss_rate_ds_weight', 1.0)
        self.loss_rate_new_weight = kwargs.get('loss_rate_new_weight', 1.0)
        ...
        self.lazy_residual_weight = kwargs.get('lazy_residual_weight', 0.1)
...
class ICLTransformer(nn.Module):
    def __init__(self, D_tok, d, heads, layers, d_ff, drop=0.0, max_demos=5):
        super().__init__()
        self.max_demos = max_demos
        self.proj_in = nn.Linear(D_tok, d); self.ln_in = nn.LayerNorm(d)
        # Token type IDs: 0=demo_z (state), 1=demo_c (label), 2=query_z
        self.type_emb = nn.Embedding(3, d)
        # Demo index positional IDs: 0..(l-1) for demos, l for query
        self.demo_pos_emb = nn.Embedding(max_demos + 1, d)
    ...
    def forward(self, seq, token_type_ids=None, demo_pos_ids=None):
        L = seq.size(1)
        mask = torch.triu(torch.ones(L,L,device=seq.device,dtype=torch.bool), diagonal=1)
        x = self.proj_in(seq)
        if token_type_ids is not None:
            x = x + self.type_emb(token_type_ids)
        if demo_pos_ids is not None:
            x = x + self.demo_pos_emb(demo_pos_ids)
        x = self.ln_in(x)
...
class PilotICLModelV5(nn.Module):
    ...
    def forward(self, demo_pil_real, demo_W_real, query_pil_real, sigma2=None):
        ...
        if l > self.transformer.max_demos:
            raise ValueError(f"n_demos={l} exceeds configured max_demos={self.transformer.max_demos}")
        ...
        for i in range(l):
            ...
            type_ids.append(torch.zeros(B, device=demo_z.device, dtype=torch.long))
            demo_pos_ids.append(torch.full((B,), i, device=demo_z.device, dtype=torch.long))
            tokens.append(demo_c[:, i])  # label token
            type_ids.append(torch.ones(B, device=demo_z.device, dtype=torch.long))
            demo_pos_ids.append(torch.full((B,), i, device=demo_z.device, dtype=torch.long))
        tokens.append(query_z)           # query state token
        type_ids.append(torch.full((B,), 2, device=demo_z.device, dtype=torch.long))
        demo_pos_ids.append(torch.full((B,), l, device=demo_z.device, dtype=torch.long))
        seq = torch.stack(tokens, dim=1)  # (B, 2l+1, D_tok)
        token_type_ids = torch.stack(type_ids, dim=1)    # (B, 2l+1)
        demo_pos_ids = torch.stack(demo_pos_ids, dim=1)  # (B, 2l+1)

        # ICL Transformer (trainable)
        out = self.transformer(seq, token_type_ids=token_type_ids, demo_pos_ids=demo_pos_ids)
...
###############################################################################
@torch.no_grad()
def _select_demos_random(dataset, B, l):
    return torch.randint(0, dataset.size, (B, l), device=device)


@torch.no_grad()
def _select_demos_similarity_mmr(model, dataset, query_pil, cfg):
    B = query_pil.size(0)
    l = cfg.n_demos
    P = min(cfg.context_pool_size, dataset.size)
    if P <= 0:
        return _select_demos_random(dataset, B, l)

    assert dataset.Y_real is not None
    pool_idx = torch.randperm(dataset.size, device=device)[:P]
    pool_pil = dataset.Y_real[pool_idx]  # (P, 2*N*L_p)

    # Pilot-embedding similarity
    qz = model.pilot_enc(query_pil, sigma2=cfg.sigma2)   # (B, D)
    pz = model.pilot_enc(pool_pil, sigma2=cfg.sigma2)    # (P, D)
    qz = F.normalize(qz, dim=-1)
    pz = F.normalize(pz, dim=-1)
    sim_qp = qz @ pz.t()  # (B, P)

    k = min(max(l, cfg.context_k_cand), P)
    topv, topi = torch.topk(sim_qp, k=k, dim=1, largest=True)
    cand_global_idx = pool_idx[topi]  # (B, k)

    out = torch.empty(B, l, device=device, dtype=torch.long)
    alpha = float(cfg.mmr_alpha)
    for b in range(B):
        cand_idx_b = cand_global_idx[b]     # (k,)
        cand_sim_q = topv[b]                # (k,)
        cand_emb = pz[topi[b]]              # (k, D)
        cand_sim_cc = cand_emb @ cand_emb.t()  # (k, k)

        selected = []
        first = int(torch.argmax(cand_sim_q).item())
        selected.append(first)

        for _ in range(1, l):
            mmr_score = cand_sim_q.clone()
            max_red = cand_sim_cc[:, selected].max(dim=1).values
            mmr_score = alpha * mmr_score - (1.0 - alpha) * max_red
            mmr_score[selected] = -1e9
            nxt = int(torch.argmax(mmr_score).item())
            selected.append(nxt)

        out[b] = cand_idx_b[torch.tensor(selected, device=device, dtype=torch.long)]
    return out
...
def select_demos(model, dataset, query_pil, cfg):
    B = query_pil.size(0)
    l = cfg.n_demos
    if int(getattr(cfg, 'is_simi_context', 0)) == 1:
        return _select_demos_similarity_mmr(model, dataset, query_pil, cfg)
    return _select_demos_random(dataset, B, l)
...
def train(cfg):
    ...
    for epoch in range(cfg.total_epochs):
        ...
        ep_mse, ep_rate_sum, ep_rate_cnt, ep_add, ep_n = 0., 0., 0, 0, 0

        # Pools from current dataset.
        assert ds.is_sup is not None
        all_pool = torch.arange(ds.size, device=device)
        ...
        for step in range(cfg.steps_per_epoch):
            ...
            # Query batch for supervised-MSE branch: sample from ALL dataset points
            # (includes both original supervised and admitted unsupervised samples).
            q_idx = all_pool[torch.randint(0, all_pool.numel(), (B,), device=device)]
            q_H = ds.H[q_idx]
            q_W_gt_real = ds.W_real[q_idx]
            q_is_unsup_ds = ~ds.is_sup[q_idx]
            q_is_sup_ds = ~q_is_unsup_ds
            ...
            mse_all = mse_per.mean()

            # Rate loss from unsupervised samples already in dataset.
            if q_is_unsup_ds.any():
                rate_loss_ds_unsup = -rate_pred[q_is_unsup_ds].mean()
            else:
                rate_loss_ds_unsup = torch.zeros((), device=device, dtype=rate_pred.dtype)

            # Additional rate loss from newly generated unsupervised datapoints.
            n_new_unsup = int(round(B * cfg.new_unsup_rate_ratio * r)) if phase == 2 else 0
            qn_rate_pred = None
            if n_new_unsup > 0:
                qn_H = generate_channel(n_new_unsup, cfg.K, cfg.N,
                                        cfg.ch_n_clusters, cfg.ch_n_rays, cfg.ch_spread_deg)
                qn_Y = pilot_observe(qn_H, Phi, cfg.sigma2)
                qn_pil = pilot_to_real(qn_Y)
                ndi = select_demos(model, ds, qn_pil, cfg)
                ndp = ds.Y_real[ndi]
                ndw = ds.W_real[ndi]
                qn_H_ls = ls_channel_est(qn_Y, Phi)
                qn_W_base = mmse_beamformer(qn_H_ls, cfg.P_max, cfg.sigma2)
                qn_dW = model(ndp, ndw, qn_pil, sigma2=cfg.sigma2)
                qn_W_hat = power_normalize(qn_W_base + qn_dW, cfg.P_max)
                qn_rate_pred = compute_sum_rate(qn_H, qn_W_hat, cfg.sigma2)
                rate_loss_new_unsup = -qn_rate_pred.mean()
            else:
            ...
            res_tgt_all = res_tgt_per.mean()

            if phase == 1:
                loss = (
                    cfg.loss_mse_weight * mse_all
                    + cfg.sup_residual_target_weight * res_tgt_all
                    + cfg.lazy_residual_weight * lazy_pen
                )
            else:
                # In Phase 2, compose MSE + dataset-unsup-rate + fresh-unsup-rate
                # with calibrated scaling for rate terms.
            ...
            with torch.no_grad():
                if phase == 2:
                    # Candidate pool size follows the configured fresh-unsup ratio.
                    n_uns_batch = int(round(B * cfg.new_unsup_rate_ratio * r))
            ...
            ep_mse += mse_all.item()
            # Train-rate metric policy:
            # phase1 -> average on supervised dataset queries;
            # phase2 -> average on in-dataset unsupervised queries;
            #           if none exist yet, fallback to fresh-unsup query rates.
            if phase == 1:
                rate_mask = q_is_sup_ds
                if rate_mask.any():
                    ep_rate_sum += rate_pred[rate_mask].sum().item()
                    ep_rate_cnt += int(rate_mask.sum().item())
            else:
                if q_is_unsup_ds.any():
                    ep_rate_sum += rate_pred[q_is_unsup_ds].sum().item()
                    ep_rate_cnt += int(q_is_unsup_ds.sum().item())
                elif qn_rate_pred is not None and qn_rate_pred.numel() > 0:
                    ep_rate_sum += qn_rate_pred.sum().item()
                    ep_rate_cnt += int(qn_rate_pred.numel())
            ep_n += 1
        ...
        am = ep_mse/max(1,ep_n)
        ar = ep_rate_sum/max(1,ep_rate_cnt)
        dt = time.time()-t0
...
if __name__ == "__main__":
    cfg = Config(
        ...
        # Token / compression
        D_tok=256,
        # Context selection switch: 0=random, 1=similarity+MMR
        is_simi_context=1,
        ...
        # Training
        batch_size=64, lr=1.5e-4, lr_min=5e-5, weight_decay=1e-4,
        initial_ds_size=1024, wmmse_iters=100, wmmse_lr=0.03, wmmse_label_restarts=2,
        unsup_scale=0.01,
        # Anti-lazy residual regularization
        lazy_residual_min_ratio=0.20,
        lazy_residual_weight=0.1,
        sup_residual_target_weight=0.25,
        # Curriculum
        phase1_epochs=50, phase2_epochs=1000, r_max=0.85, steps_per_epoch=80,
        # Dual-threshold
        boot_alpha_start=0.65, boot_alpha_end=0.9,
        boot_beta_start=0.65, boot_beta_end=0.9,
        max_ds_size=100000,
        # Pruning
