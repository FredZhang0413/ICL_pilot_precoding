...
class Config:
    def __init__(self, **kwargs):
        # System
        self.K = kwargs.get('K', 32)  ## user number
        self.N = kwargs.get('N', 32)  ## antenna number
        self.L_p = kwargs.get('L_p', 20)  ## pilot length (limited)
        self.P_max = kwargs.get('P_max', 1.0)  ## maximum power of pilots
        self.SNR_dB = kwargs.get('SNR_dB', 15)  ## signal-to-noise ratio in dB
        ...
        # Token dimensions
        self.token_dim = 4 * self.K      # state/label token size per user
        self.label_dim = 2 * self.K      # (p, lambda) before padding per user

        # ICL
        self.n_demos = kwargs.get('n_demos', 5) ## context length
        # Similarity-based context selection
        self.context_k_cand = kwargs.get('context_k_cand', 20)  ## number of candidate similar contexts
        self.context_pool_size = kwargs.get('context_pool_size', 512)  ## size of the context pool
        self.mmr_alpha = kwargs.get('mmr_alpha', 0.65)  ## alpha parameter in MMR 

        # Pilot Encoder
        self.encoder_hidden = kwargs.get('encoder_hidden', 128)  ## hidden layer width of pilot encoder

        # Transformer (no PE, with causal mask)
        self.d_model = kwargs.get('d_model', 512)  ## Transformer model-dimension
        ...
        # Dataset
        self.initial_dataset_size = kwargs.get('initial_dataset_size', 1600)  ## small supervised dataset
        self.opt_iters = kwargs.get('opt_iters', 500)   # Adam iters for (p,lam) labels
        self.opt_lr = kwargs.get('opt_lr', 0.03)

        # Robust matched labels: optimize (p,lam) on H_hat, evaluate rates on true H (very important!)
        self.use_robust_labels = kwargs.get('use_robust_labels', True)

        # Curriculum: our multi-stage training scheme
        ...
        # Quantile-adaptive admission threshold (replaces fixed baseline scaling)
        self.boot_q_start = kwargs.get('boot_q_start', 0.65)
        self.boot_q_end = kwargs.get('boot_q_end', 0.90)
        self.boot_thresh_ema_beta = kwargs.get('boot_thresh_ema_beta', 0.2)

        # Forgetting / bottom-pruning (mainly for self-bootstrapped noisy labels)
        self.forget_every_epochs = kwargs.get('forget_every_epochs', 5)
        self.forget_unsup_drop_start = kwargs.get('forget_unsup_drop_start', 0.0)
        self.forget_unsup_drop_end = kwargs.get('forget_unsup_drop_end', 0.1)
        self.forget_min_unsup_keep = kwargs.get('forget_min_unsup_keep', 2048)  ## minimum number of unsupervised samples to keep
...
###############################################################################
@torch.no_grad()
def _eval_rate(H, p_logits, lam_logits, P_max, sigma2): ## use true channels to evaluate the sum rate
...
class PilotICLModel(nn.Module):
    ...
    def _label_token(self, p, lam):
        ...
        return tok ## standard solution token design
    ...
    def forward(self, demo_pilots, demo_p, demo_lam, query_pilot):
        ...
        """
        B, l, pd = demo_pilots.shape  ## pd=2*N*L_p
        all_pil  = torch.cat([demo_pilots.reshape(B*l, pd), query_pilot], dim=0) ## concatenate all pilots
        all_z = self.encoder(all_pil) ## encode all pilots, with size (B*(l+1), token_dim)
        demo_z = all_z[:B*l].reshape(B, l, self.token_dim) ## previous l as context state tokens
        query_z = all_z[B*l:] ## the last one as the current query state token

        tokens = []
        for i in range(l): ## construct tokens
        ...
        return self._extract(out[:, -1])  ## extract the predicted (p, lam) from the transformer output
...
class DynDataset:
    def __init__(self, max_sz=30000):
        self.max_sz = max_sz  ## max size
        self.H = self.p = self.lam = self.rates = None
        self.is_sup = None  ## whether is constructed by supervised labels
    ...
    def add(self, H, p, lam, rates, supervised=True):
        ...
        sup_flag = torch.full((H.size(0),), bool(supervised), device=H.device, dtype=torch.bool)
        if self.H is None:
            self.H, self.p, self.lam, self.rates = H, p, lam, rates
            self.is_sup = sup_flag
        else:
            assert self.H is not None and self.p is not None and self.lam is not None and self.rates is not None
            assert self.is_sup is not None
            self.H = torch.cat([self.H, H])
            self.p = torch.cat([self.p, p])
            self.lam = torch.cat([self.lam, lam])
            self.rates = torch.cat([self.rates, rates])
            self.is_sup = torch.cat([self.is_sup, sup_flag])
        m = H.size(0)
        ...
        if self.H.size(0) > self.max_sz:
            ...
            assert self.is_sup is not None
            self.H, self.p, self.lam, self.rates, self.is_sup = [
                t[idx] for t in [self.H, self.p, self.lam, self.rates, self.is_sup]
            ]
        assert self.is_sup is not None
    ...
    def prune_unsup_bottom(self, drop_ratio: float, min_unsup_keep: int = 0) -> int:
        """
        Forgetting mechanism: drop the worst unsupervised samples by rate.
        Keeps all supervised samples; removes bottom `drop_ratio` among unsup,
        while preserving at least `min_unsup_keep` unsupervised samples.
        Returns number of dropped samples.
        """
        if self._n == 0 or drop_ratio <= 0.0:
            return 0
        assert self.H is not None and self.p is not None and self.lam is not None and self.rates is not None
        assert self.is_sup is not None

        unsup_idx = torch.where(~self.is_sup)[0]
        n_unsup = int(unsup_idx.numel())
        if n_unsup == 0:
            return 0

        removable = max(0, n_unsup - int(min_unsup_keep))
        n_drop = min(int(n_unsup * drop_ratio), removable)
        if n_drop <= 0:
            return 0

        unsup_rates = self.rates[unsup_idx]
        worst_local = torch.topk(unsup_rates, k=n_drop, largest=False).indices
        drop_global = unsup_idx[worst_local]

        keep_mask = torch.ones(self._n, device=self.rates.device, dtype=torch.bool)
        keep_mask[drop_global] = False

        self.H = self.H[keep_mask]
        self.p = self.p[keep_mask]
        self.lam = self.lam[keep_mask]
        self.rates = self.rates[keep_mask]
        self.is_sup = self.is_sup[keep_mask]

        self._n = int(self.H.size(0))
        self.n_sup = int(self.is_sup.sum().item())
        self.n_unsup = int((~self.is_sup).sum().item())
        return n_drop
...
@torch.no_grad()
def select_demo_indices_similarity(model: PilotICLModel,
                                   dataset: DynDataset,
                                   query_pil: torch.Tensor,
                                   Phi: torch.Tensor,
                                   cfg: Config,
                                   l: int) -> torch.Tensor:
    """
    Similarity-first + MMR diversity selection.

    Step1: build candidate pool, encode pilots, retrieve top-K by cosine similarity.
    Step2: run MMR to pick final l demos balancing similarity and diversity.
    Returns demo indices with shape (B, l).
    """
    B = query_pil.size(0)
    assert dataset.size > 0 and dataset.H is not None

    # Small dataset fallback (allow duplicates)
    if dataset.size <= l:
        return torch.randint(0, dataset.size, (B, l), device=query_pil.device)

    pool_size = min(cfg.context_pool_size, dataset.size)
    if pool_size < dataset.size:
        pool_idx = torch.randperm(dataset.size, device=query_pil.device)[:pool_size]
    else:
        pool_idx = torch.arange(dataset.size, device=query_pil.device)

    # Candidate embeddings from compressed pilot representations
    H_pool = dataset.H[pool_idx]
    Y_pool = pilot_observe(H_pool, Phi, cfg.sigma2)
    pil_pool = pilot_to_real(Y_pool)

    z_query = model.encoder(query_pil)            # (B, D)
    z_pool = model.encoder(pil_pool)              # (P, D)
    z_query = F.normalize(z_query, dim=-1)
    z_pool = F.normalize(z_pool, dim=-1)

    sim_qp = z_query @ z_pool.transpose(0, 1)     # (B, P)

    k_cand = min(max(l, cfg.context_k_cand), pool_size)
    alpha = float(cfg.mmr_alpha)
    cand_idx = torch.topk(sim_qp, k=k_cand, dim=1, largest=True, sorted=True).indices  # (B, Kc)

    chosen_local = torch.zeros(B, l, dtype=torch.long, device=query_pil.device)
    for b in range(B):
        c = cand_idx[b]                           # (Kc,)
        c_emb = z_pool[c]                         # (Kc, D)
        q_sim = sim_qp[b, c]                      # (Kc,)
        cc_sim = c_emb @ c_emb.transpose(0, 1)    # (Kc, Kc)

        first = torch.argmax(q_sim)
        selected = [int(first.item())]
        selected_mask = torch.zeros(k_cand, dtype=torch.bool, device=query_pil.device)
        selected_mask[first] = True

        for _ in range(1, l):
            max_sim_sel = cc_sim[:, selected].max(dim=1).values
            mmr_score = alpha * q_sim - (1.0 - alpha) * max_sim_sel
            mmr_score[selected_mask] = -1e9
            nxt = torch.argmax(mmr_score)
            selected.append(int(nxt.item()))
            selected_mask[nxt] = True

        chosen_local[b] = c[torch.tensor(selected, device=query_pil.device, dtype=torch.long)]

    # map from pool-local indices to dataset-global indices
...
def evaluate_model(model: PilotICLModel, dataset: DynDataset,
    ...
    model.eval()
    assert dataset.H is not None and dataset.p is not None and dataset.lam is not None
    ...
    for start in range(0, B_total, bs):
        ...
        # Context from dataset (similarity-based + MMR diversity)
        idx = select_demo_indices_similarity(model, dataset, pil, Phi, cfg, cfg.n_demos)
...
def train(cfg: Config):
    ...
    label_mode = "noisy-CSI labels (opt on H_hat)" if cfg.use_robust_labels else "true-CSI labels (opt on H)"
    print(f"\nGenerating initial dataset (M0={cfg.initial_dataset_size}, using {label_mode})...")
    ...
    for s in range(0, cfg.initial_dataset_size, gen_bs):
        ...
        H_b = generate_channel(e - s, cfg.K, cfg.N,
                       ...
                       n_rays_per_cluster=cfg.ch_n_rays_per_cluster,
                       angle_spread_deg=cfg.ch_angle_spread_deg)
        # Recovered noisy channel from pilots
        Y_b = pilot_observe(H_b, Phi, cfg.sigma2)
        H_b_hat = mmse_channel_est(Y_b, Phi, cfg.sigma2)

        # Label generation:
        #   robust mode: optimize on H_hat (matched with inference-time reconstruction)
        #   legacy mode: optimize on true H
        H_design = H_b_hat if cfg.use_robust_labels else H_b
        p_b, lam_b, _ = generate_optimal_params(H_design, cfg.P_max, cfg.sigma2,
                                                n_iters=cfg.opt_iters, lr=cfg.opt_lr)

        # Sum-rate always evaluated on TRUE H;
        # precoder structure reconstructed on design channel (H_hat in robust mode)
        with torch.no_grad():
            W_b = reconstruct_precoder(H_design, p_b, lam_b, cfg.sigma2)
    ...
    for epoch in range(cfg.total_epochs):
        ...
        # Quantile-adaptive threshold scheduling (strictness increases over epochs)
        q_prog = epoch / max(1, cfg.total_epochs - 1)
        q_admit = cfg.boot_q_start + (cfg.boot_q_end - cfg.boot_q_start) * q_prog
        q_admit = float(max(0.0, min(0.99, q_admit)))
        ...
        for step in range(cfg.steps_per_epoch):
            ...
            # Pilot signals + MMSE estimates
            q_Y = pilot_observe(q_H, Phi, cfg.sigma2)
            q_pil = pilot_to_real(q_Y)
            q_H_hat = mmse_channel_est(q_Y, Phi, cfg.sigma2)

            # Demo pairs from dataset (similarity-based + MMR diversity)
            d_idx = select_demo_indices_similarity(model, dataset, q_pil, Phi, cfg, l)
            ...
            with torch.no_grad():
                if is_unsup.any():
                    ...
                    step_thresh = torch.quantile(unsup_rates.detach(), q_admit).item()
                    if running_bootstrap_thresh is None:
                        running_bootstrap_thresh = step_thresh
                    else:
                        beta = cfg.boot_thresh_ema_beta
                        running_bootstrap_thresh = (1.0 - beta) * running_bootstrap_thresh + beta * step_thresh

                    good = unsup_rates > float(running_bootstrap_thresh)
                    if good.any():
        ...
        # Periodic forgetting: remove bottom unsupervised samples by rate
        n_dropped = 0
        if (epoch + 1) % max(1, cfg.forget_every_epochs) == 0 and epoch >= cfg.phase1_epochs:
            f_prog = (epoch - cfg.phase1_epochs) / max(1, cfg.phase2_epochs - 1)
            drop_ratio = cfg.forget_unsup_drop_start + \
                (cfg.forget_unsup_drop_end - cfg.forget_unsup_drop_start) * f_prog
            n_dropped = dataset.prune_unsup_bottom(drop_ratio, cfg.forget_min_unsup_keep)
        ...
        print(f"{epoch+1:3d} {phase:>2d} {r:.2f} | "
              ...
              f"(thr={0.0 if running_bootstrap_thresh is None else running_bootstrap_thresh:.2f}, "
              f"drop={n_dropped}, {dt:.1f}s)", flush=True)
    ...
    _save_training_rates_pt(history, cfg, save_path='training_rates_v2.pt')
...
def _save_training_rates_pt(history: Dict, cfg: 'Config',
                            save_path: str = 'training_rates_v2.pt') -> None:
    """Save per-epoch training/test curves to a .pt file for later analysis."""
    payload = {
        'epochs': torch.arange(1, len(history['train_rate']) + 1, device='cpu'),
        'train_rate': torch.tensor(history['train_rate'], dtype=torch.float32),
        'test_rate': torch.tensor(history['test_rate'], dtype=torch.float32),
        'mse': torch.tensor(history['mse'], dtype=torch.float32),
        'ds_size': torch.tensor(history['ds_size'], dtype=torch.int64),
        'ep_added': torch.tensor(history['ep_added'], dtype=torch.int64),
        'phase': torch.tensor(history['phase'], dtype=torch.int64),
        'meta': {
            'K': cfg.K,
            'N': cfg.N,
            'L_p': cfg.L_p,
            'SNR_dB': cfg.SNR_dB,
            'ch_n_clusters': cfg.ch_n_clusters,
            'ch_n_rays_per_cluster': cfg.ch_n_rays_per_cluster,
            'ch_angle_spread_deg': cfg.ch_angle_spread_deg,
        }
    }
    torch.save(payload, save_path)
    print(f"  Training rates saved → {save_path}")
...
if __name__ == "__main__":
    cfg = Config(
        ...
        # Sparse channel params (unified control)
        ch_n_clusters=3, ## cluster number
        ch_n_rays_per_cluster=5, ### rays per cluster
        ch_angle_spread_deg=10.0, ### angle spread in degrees
        n_demos=5,              # longer ICL context (was 20)
        encoder_hidden=256,
        ...
        r_max=0.85,
        unsup_scale=0.005,
        tau_start=80, tau_end=100,
        boot_q_start=0.65, boot_q_end=0.90, boot_thresh_ema_beta=0.2,
        forget_every_epochs=5,
        forget_unsup_drop_start=0.00, forget_unsup_drop_end=0.1,
        forget_min_unsup_keep=2048,
