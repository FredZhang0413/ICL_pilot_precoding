...
def train(cfg):
    ...
    for epoch in range(cfg.total_epochs):
        ...
        if epoch < cfg.phase1_epochs:
            ...
            phase = 2
            # Requested step schedule for unsupervised query ratio:
            # 0.25 for 50 epochs, 0.5 for next 50, 0.75 for next 50,
            # then pure unsupervised (r=100) for the remaining epochs.
        ...
        for step in range(cfg.steps_per_epoch):
            ...
            if phase == 1:
                ...
                if r >= 1.0:
                    loss = hybrid_rate_scale * rate_loss
                else:
                    loss = w_mse * mse_sup + w_rate * (hybrid_rate_scale * rate_loss)
            ...
            with torch.no_grad():
                if is_unsup.any():
                    ...
                    # Per-instance benchmark: MMSE-imp (estimated channel from noisy pilot)
                    q_H_u = q_H[is_unsup]
                    q_Y_u = q_Y[is_unsup]
                    H_hat_u = mmse_channel_est(q_Y_u, Phi, cfg.sigma2)
                    W_mmse_imp_u = mmse_beamformer(H_hat_u, cfg.P_max, cfg.sigma2)
                    mmse_imp_r_u = compute_sum_rate(q_H_u, W_mmse_imp_u, cfg.sigma2)
                    per_ok = ur > alpha_t * mmse_imp_r_u
                    ...
                    if good.any():
                        ...
                        # Store the exact noisy pilot used to produce W_hat
                        # to avoid pilot/label mismatch.
                        ds.add(q_H[gi], q_pil[gi], bf_to_real(W_hat[gi]),
                               rate_pred[gi], mmse_rates=mmse_imp_r_u[good], supervised=False)
...
if __name__ == "__main__":
    cfg = Config(
        ...
        initial_ds_size=1024, wmmse_iters=500, wmmse_lr=0.03, wmmse_label_restarts=2,
        unsup_scale=0.01,
        # Curriculum
        phase1_epochs=50, phase2_epochs=2000, r_max=0.85, steps_per_epoch=80,
        # Dual-threshold
