...
class Config:
    def __init__(self, **kwargs):
        ...
        self.use_dynamic_unsup_scale = kwargs.get('use_dynamic_unsup_scale', True)
        self.unsup_scale_min = kwargs.get('unsup_scale_min', 0.001)
        self.unsup_scale_max = kwargs.get('unsup_scale_max', 0.05)
        self.balance_ema_beta = kwargs.get('balance_ema_beta', 0.95)
...
class CausalBlock(nn.Module):
    def __init__(self, d: int, heads: int, d_ff: int, drop: float = 0.0):
        ...
        self.ff_in = nn.Linear(d, 2 * d_ff)
        self.ff_out = nn.Linear(d_ff, d)
        self.ff_drop = nn.Dropout(drop)
        self.gamma_attn = nn.Parameter(torch.ones(d) * 1e-2)
        self.gamma_ff = nn.Parameter(torch.ones(d) * 1e-2)

    def forward(self, x, mask):
        h = self.ln1(x)
        attn_out = self.attn(h, h, h, attn_mask=mask)[0]
        x = x + attn_out * self.gamma_attn.view(1, 1, -1)

        h2 = self.ln2(x)
        u, g = self.ff_in(h2).chunk(2, dim=-1)
        ff = self.ff_out(u * F.gelu(g))
        ff = self.ff_drop(ff)
        return x + ff * self.gamma_ff.view(1, 1, -1)
...
class PilotICLModel(nn.Module):
    def __init__(self, cfg: Config):
        ...
        self.type_embed = nn.Embedding(3, cfg.token_dim)
    ...
    def forward(self, demo_pilots, demo_p, demo_lam, query_pilot):
        ...
        for i in range(l):
            ...
            type_ids.append(0)  # demo state token
            tokens.append(self._label_token(demo_p[:, i], demo_lam[:, i]))
            type_ids.append(1)  # demo label token
        tokens.append(query_z)
        type_ids.append(2)      # query state token
        seq = torch.stack(tokens, dim=1)  # (B, 2l+1, tok)
        type_tensor = torch.tensor(type_ids, device=seq.device, dtype=torch.long)
        seq = seq + self.type_embed(type_tensor).unsqueeze(0)
...
def train(cfg: Config):
    ...
    for epoch in range(cfg.total_epochs):
        ...
        # Admission threshold: tau * B4 (Opt with noisy/estimated CSI)
        thresh = (tau / 100.0) * baselines['opt_imperfect']
        ...
        for step in range(cfg.steps_per_epoch):
            ...
            # Dynamic unsup scale to balance gradient magnitudes across phases/distributions
            with torch.no_grad():
                cur_mse_mag = mse_per[~is_unsup].mean() if (~is_unsup).any() else None
                cur_rate_mag = rate_pred[is_unsup].mean() if is_unsup.any() else None

                if cur_mse_mag is not None:
                    ema_mse_mag = cur_mse_mag if ema_mse_mag is None else (
                        cfg.balance_ema_beta * ema_mse_mag + (1 - cfg.balance_ema_beta) * cur_mse_mag
                    )
                if cur_rate_mag is not None:
                    ema_rate_mag = cur_rate_mag if ema_rate_mag is None else (
                        cfg.balance_ema_beta * ema_rate_mag + (1 - cfg.balance_ema_beta) * cur_rate_mag
                    )

            if cfg.use_dynamic_unsup_scale and (ema_mse_mag is not None) and (ema_rate_mag is not None):
                dyn_scale = (ema_mse_mag / (ema_rate_mag + 1e-8)).clamp(
                    cfg.unsup_scale_min, cfg.unsup_scale_max
                )
            else:
                dyn_scale = torch.tensor(cfg.unsup_scale, device=device)

            loss_per = torch.where(is_unsup, -rate_pred * dyn_scale, mse_per)
            loss = loss_per.mean()
        ...
        if epoch == cfg.phase1_epochs:
            ...
            dyn_scale_show = dyn_scale.item() if isinstance(dyn_scale, torch.Tensor) else float(dyn_scale)
            ...
            print(f"  [Loss balance] |rate|/|MSE| ~ {ds_rate_avg/max(avg_mse,1e-8):.0f}x  "
                f"-> unsup_scale={dyn_scale_show:.4f}  "
                f"(scaled rate loss ~ {ds_rate_avg*dyn_scale_show:.4f})\n", flush=True)
...
if __name__ == "__main__":
    cfg = Config(
        K=32, N=32, L_p=15,      # L_p < K: realistic under-determined pilot regime
        P_max=1.0, SNR_dB=20,    # 20 dB keeps imperfect-CSI gap reasonable with L_p=20
        ...
        r_max=0.85,
        unsup_scale=0.005,
        tau_start=70, tau_end=100,
        max_dataset_size=70000,
        n_test=300,
    )
    train(cfg)
