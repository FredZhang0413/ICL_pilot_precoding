...
class DirectPilotBFModel(nn.Module):
    ...
    Direct mapping: W_hat = f_Transformer(Y), no MMSE-reference residual.
    """
    ...
    def forward(self, Y_complex, sigma2=None):
        ...
        Returns: W_hat (B, N, K) complex — direct beamformer
        """
        B = Y_complex.size(0)
        ...
        W_hat = real_to_bf(w_real, self.N, self.K)
        return power_normalize(W_hat, self.P_max)
...
def train(cfg):
    ...
    for epoch in range(cfg.total_epochs):
        ...
        for step in range(cfg.steps_per_epoch):
            ...
            # Direct Pilot-to-BF mapping (no MMSE-imp/LS reference)
            W_hat = model(q_Y, sigma2=cfg.sigma2)
        ...
        with torch.no_grad():
            for s in range(0, H_test.size(0), cfg.batch_size):
                ...
                Y = pilot_observe(H, Phi, cfg.sigma2)
                W_hat = model(Y, sigma2=cfg.sigma2)
    ...
    save_and_plot_rates(hist['train'], hist['test'], tag='a1a_no_icl')
    return model, bl

if __name__=="__main__":
    cfg=Config(K=32,N=32,L_p=20,SNR_dB=20,d_model=512,n_heads=8,n_layers=6,d_ff=1024,
               ds_size=5000,phase1_epochs=50,phase2_epochs=1000,steps_per_epoch=80,
               batch_size=64,lr=2e-4,n_test=200)
    train(cfg)
