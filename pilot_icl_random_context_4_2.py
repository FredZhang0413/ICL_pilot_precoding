...
def train(cfg):
    ...
    for epoch in range(cfg.total_epochs):
        ...
        for step in range(cfg.steps_per_epoch):
            ...
            # Unsupervised rate-query tokens can only come from unsupervised pool.
            if uns_pool.numel() == 0:
                is_unsup = torch.zeros(B, device=device, dtype=torch.bool)
            elif r >= 1.0:
            ...
            # Unsupervised rate-loss queries: only from already admitted unsupervised datapoints.
            if uns_pool.numel() > 0:
                uidx = uns_pool[torch.randint(0, uns_pool.numel(), (B,), device=device)]
                q_H_unsup = ds.H[uidx]
                q_W_unsup_real = ds.W_real[uidx]
            else:
                q_H_unsup = q_H_sup
                q_W_unsup_real = q_W_gt_real
            ...
            with torch.no_grad():
                if phase == 2:
                    ...
                    cand_H = generate_channel(B, cfg.K, cfg.N, cfg.ch_n_clusters,
                                              cfg.ch_n_rays, cfg.ch_spread_deg)
